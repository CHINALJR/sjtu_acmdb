...
public class BufferPool {
    ...
    private ConcurrentHashMap<PageId, Page> _caches;
    /**
    ...
    public BufferPool(int numPages) {
        _cacheSize = numPages;
        _caches = new ConcurrentHashMap<PageId, Page>();
		// some code goes here
    }
    ...
    public  Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        Page diskPage = Database.getCatalog().getDatabaseFile(pid.getTableId()).readPage(pid);
    ...
    public void insertTuple(TransactionId tid, int tableId, Tuple t)
        ...
        ArrayList<Page> dpList = f.insertTuple(tid, t);
        	...
        	for (Page p : dpList) {
            		PageId pid = p.getId();
            		if (!_caches.containsKey(pid) && _caches.size() == _cacheSize) evictPage();
            		_caches.put(pid, p);
            		_caches.get(pid).markDirty(true, tid);
        	}
    ...
    public  void deleteTuple(TransactionId tid, Tuple t)
	  ...
	  DbFile f = Database.getCatalog().getDatabaseFile(t.getRecordId().getPageId().getTableId());
        ArrayList<Page> dpList = f.deleteTuple(tid, t);//iterate through affectedPages and markDirty        
	for (Page p : dpList) {
            PageId pid = p.getId();
            if (!_caches.containsKey(pid) && _caches.size() == _cacheSize) evictPage();
            _caches.put(pid, p);
    ...
    */
	...
	Catalog catalog = Database.getCatalog();
        DbFile pageFile = catalog.getDatabaseFile(pid.getTableId());
    ...
    private synchronized  void evictPage() throws DbException {
        ...
        for (PageId pid : _caches.keySet()) {
            if (_caches.get(pid).isDirty() == null) {
                cleanPages.add(pid);
            }
        }
        ...
        try {
            assert _caches.get(vic).isDirty() == null : "Evict a dirty page!";
            flushPage(vic);
        } catch (Exception e) {
            e.printStackTrace();
        }
        _caches.remove(vic); // remove from pool
